                                  ...
                                  const size_t numBins)
{
  ...
  /*Here are the steps you need to implement
	...
	myprefixsum<<<gridSize, blockSize, 2*numBins*sizeof(int)>>>(d_logLumBin, d_cdf, logLumRange, numRows, numCols, numBins);
...
__global__ myprefixsum(int * d_logLumBin, unsigned int * d_cdf, float logLumRange, size_t nowRows, size_t numCols, size_t numBins)
{
  __share__ unsigned int *d_temp;
  int tid = threadIdx.x;
  int pout = 0; pin = 1;
  d_temp[pout*numBins+tid] = (tid>0)? d_logLumBin[tid - 1] : 0;
  ...
  for (int offset= 1; offset < numBins, offset *=2) {
    ...
    if (tid >= offset)
      d_temp[pout*numBins+tid] += d_temp[pin*numBins+tid-offset];
    else 
      d_temp[pout*numBins+tid] = d_temp[pin*numBins+tid];

    __syncthreads();

  }

  d_cdf[tid] = d_temp[pout*numBins+tid];
